...
print(len(df))
df_region = (df.loc[(df['lat'] > 37.5) & (df['lat'] < 40.5) & (df['lng'] < -82.5) & (df['lng'] > -86)])
print(len(df_region))
df_large = df_region[df_region['population'] > 500] #could change this to remove smaller towns if its too cluttered
print(len(df_large))
df_large.to_csv('filtered_cities.csv')
'''
df_large = pd.read_csv('filtered_cities.csv')
logo_path= 'cincyweathernobg.png'
logo = mpimg.imread(logo_path)
'''
roads_box = box(-87.5, 37.0, -81.0, 42.0)
roads = gpd.read_file("ne_10m_roads/ne_10m_roads.shp")
REGIONAL_ROADS_SHP_PATH = "regional_roads_filtered.shp"
try:
    print(f"Saving filtered regional roads to {REGIONAL_ROADS_SHP_PATH}...")
    regional_roads.to_file(REGIONAL_ROADS_SHP_PATH)
    print(f"Successfully saved {REGIONAL_ROADS_SHP_PATH}")
except Exception as e:
    print(f"Error saving Shapefile: {e}")
print(roads['name'])
'''
regional_roads = gpd.read_file('regional_roads_filtered.shp') #open filtered roads dataset
interstates = regional_roads[regional_roads['name'].str.contains(r'\b(?:75|71|275|70|74|471|670|675|270)\b', na = False)] #check for any of these names in dataset
...
def plot_alert_polygon(alert, output_path="alert_map.png"):
    ...
    try:
        ...
        interstates.plot(ax=ax, linewidth = 1, edgecolor='blue', transform = ccrs.PlateCarree())
        regional_roads.plot(ax=ax, linewidth= 0.35, edgecolor= "#f14646", transform = ccrs.PlateCarree())
        ...
        ax.text(0.012, 0.986, f"Issued {formatted_issued_time} by {issuing_office}", 
        ...
        if geom.geom_type == 'Polygon':
            if alert_type == "Severe Thunderstorm Warning":
                ...
                ax.plot(x, y, color="#ffee00", linewidth=2, transform=ccrs.PlateCarree())
                ax.fill(x, y, facecolor="#ffff006e")
            elif alert_type == 'Tornado Warning':
                x, y = geom.exterior.xy
        ...
        torPossible = alert['properties']['parameters'].get('tornadoDetection', ['n/a'])[0] #string, possible for svr; radar-indicated, radar-confirmed, need to see others for tor warning
        floodImpact = alert['properties']['parameters'].get('flashFloodDamageThreat', ['n/a'])[0] #string, default level (unsure what this returns), considerable, catastophic
        tStormSeverity = alert['properties']['parameters'].get('thunderstormDamageThreat', ['n/a'])[0] 
        
        print(maxWind, maxHail, torPossible, floodImpact)
        hazard_details = [
            (maxWind, 'Max. Wind Gusts', ""),
            (maxHail, 'Max. Hail Size', "in"),
            (torPossible, 'Tornado', ""),
            (floodImpact, 'Flood Threat', "")

        ]
        details_text = '\n'.join(
            f"{label}: {value}{suffix}" for value, label, suffix in hazard_details if value != "n/a"
        )
        ...
        desc = alert['properties'].get('description', ['n/a'])
        ...
        print(f"Map saved to {output_path} in {elapsed_plot_time:.2f}s. Total script time: {elapsed_total_time:.2f}s")
        print(statement)
        return output_path, statement
    except Exception as e:
        print(f"Error plotting alert geometry: {e}")
        return None


plot_alert_polygon(test_alert4)